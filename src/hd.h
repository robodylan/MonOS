...
void hd_rw(unsigned int LBA, unsigned int com, unsigned int sectors_to_access, char *buf) 
{
	...
	write_port(0x1F2, (unsigned char)sectors_to_access);
	write_port(0x1F3, (unsigned char)LBA);//Lowest 8-bits of LBA
	write_port(0x1F4, (unsigned char)LBA >> 8);//Next 8-bits of LBA
	write_port(0x1F5, (unsigned char)LBA >> 16);//Next 8-bits of LBA
	write_port(0x1F6, (unsigned char)(0xE0 | ((0) << 4) | ((LBA >> 24) & 0x0F)));//Last 8-bits of LBA plus drive
